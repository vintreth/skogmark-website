...
@Component
@Scope("session")
public class OldDumbWisdomGenerator implements WisdomGenerator {
    ...
    private static final Logger logger = LoggerFactory.getLogger(OldDumbWisdomGenerator.class);
    ...
    @Autowired
    public OldDumbWisdomGenerator(SentencePartDao sentencePartDao, ConjunctionDao conjunctionDao) {
        ...
        private List<E> getOrCreate() {
            if (null == cachedEntities) {
                ...
                if (null == cachedEntities || cachedEntities.isEmpty()) {
                    throw new GenerationException("Unable to retrieve entities from dao layer for " + this);
                }
            }
    ...
    public Wisdom[] generateMany(int count) {
        return generateMany(count, this::generateOne);
    }
    ...
    private void loadLocalRepository() {
        ...
        try (InputStream inputStream = Thread.currentThread().getContextClassLoader().getResourceAsStream(LOCAL_REPOSITORY)) {
            ...
            throw new GenerationException("Unable to resolve local repository file " + LOCAL_REPOSITORY, e);
        } catch (IOException e) {
            throw new GenerationException("Failure to read file" + LOCAL_REPOSITORY, e);
        }
    }
...
//        wisdom.setTemplate(template);
    ...
    private String parseTemplate(String template) {
        ...
        if (0 >= template.length()) {
            throw new GenerationException("Unable to parse empty template");
        }
    ...
    private RoleId getRoleIdByVariable(String variable) {
        ...
        if (!matcher.matches()) {
            throw new GenerationException("Unable to parse template variable " + variable);
        }
